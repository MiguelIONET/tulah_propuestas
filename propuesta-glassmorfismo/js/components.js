(function () {
    ...
    connectedCallback() {
      ...
      this.innerHTML = `
        ...
        <header class="site-header"><div class="nav-shell">
          <a class="brand" href="index.html" aria-label="Tulah — Inicio"><img src="assets/logo-tulah.svg" alt="Tulah" width="148" height="48"></a>
          <button class="menu-toggle" type="button" aria-expanded="false" aria-controls="main-menu"><span class="sr-only">Abrir menú</span><i class="bi bi-list" aria-hidden="true"></i></button>
          <nav id="main-menu" class="main-nav" aria-label="Navegación principal">${data.nav.map((item) => `<a href="${item.href}"${item.href === active ? ' aria-current="page"' : ''}>${item.label}</a>`).join('')}<a class="button button-small nav-cta" href="contacto.html">Cotizar ahora</a></nav>
        </div></header>`;
    ...
    connectedCallback() {
      this.innerHTML = `
        <footer class="site-footer"><div class="footer-grid">
          <div class="footer-brand"><img src="assets/logo-tulah.svg" alt="Tulah" width="148" height="48"><p>Equipo de seguridad y automatización industrial, regalos, globos y más.</p><a class="footer-phone" href="tel:+528114744867">${data.displayPhone}</a></div>
        ...
        </div><div class="footer-bottom"><p>© ${new Date().getFullYear()} Tulah. Todos los derechos reservados.</p><p>Seguridad que acompaña.</p></div></footer>`;
    }
  }
  ...
  class ProductCatalog extends HTMLElement {
    connectedCallback() {
      ...
      this.innerHTML = `<div class="catalog-toolbar" role="search"><label class="search-field"><span class="sr-only">Buscar productos</span><i class="bi bi-search" aria-hidden="true"></i><input type="search" value="${escapeHtml(this.state.query)}" placeholder="Buscar lentes, guantes, arneses…" autocomplete="off"></label><div class="filter-row" aria-label="Filtrar por familia"><button class="filter-chip is-active" type="button" data-group="all">Todo</button>${groups.map((group) => `<button class="filter-chip" type="button" data-group="${escapeHtml(group)}">${escapeHtml(group)}</button>`).join('')}</div></div><div class="catalog-summary" aria-live="polite"></div><div class="catalog-grid"></div>`;
    ...
    renderProducts() {
      ...
      this.querySelector('.catalog-grid').innerHTML = products.length ? products.map((product) => `<article class="product-card" id="${product.id}"><div class="product-icon" data-icon="${product.icon}">${icon(product.icon)}</div><p class="card-kicker">${escapeHtml(product.group)}</p><h2>${escapeHtml(product.name)}</h2><p>${escapeHtml(product.description)}</p><ul>${product.items.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul>${productBrands(product.id)}<a class="card-action" target="_blank" rel="noopener" href="${utils.buildWhatsAppUrl(`Hola Tulah, deseo cotizar productos de ${product.name}.`)}">Solicitar cotización <span aria-hidden="true"><i class="bi bi-arrow-up-right" aria-hidden="true"></i></span></a></article>`).join('') : `<div class="empty-state"><div class="product-icon">${icon()}</div><h2>No encontramos coincidencias</h2><p>Prueba con otra palabra o consulta todo el catálogo.</p><button class="button button-secondary" type="button">Limpiar búsqueda</button></div>`;
  ...
  class ContactForm extends HTMLElement {
    connectedCallback() {
      this.innerHTML = `<form class="contact-form" novalidate><div class="form-heading"><p class="eyebrow">Cuéntanos tu necesidad</p><h2>Preparemos tu cotización</h2><p>Completa los datos y continuaremos la conversación por WhatsApp.</p></div><div class="field-grid"><label class="field"><span>Nombre <b aria-hidden="true">*</b></span><input name="name" autocomplete="name" required placeholder="Tu nombre"><small class="field-error" id="name-error"></small></label><label class="field"><span>Correo <b aria-hidden="true">*</b></span><input name="email" type="email" autocomplete="email" required placeholder="nombre@empresa.com"><small class="field-error" id="email-error"></small></label></div><label class="field"><span>Empresa <em>opcional</em></span><input name="company" autocomplete="organization" placeholder="Nombre de tu empresa"></label><label class="field"><span>¿Qué necesitas? <b aria-hidden="true">*</b></span><textarea name="message" rows="5" required placeholder="Producto, cantidad aproximada o tipo de riesgo que buscas atender"></textarea><small class="field-error" id="message-error"></small></label><button class="button form-submit" type="submit">Enviar por WhatsApp <span aria-hidden="true"><i class="bi bi-arrow-up-right" aria-hidden="true"></i></span></button><p class="form-note">Al continuar se abrirá WhatsApp; no almacenamos tus datos en este sitio.</p></form>`;
      ...
      form.addEventListener('submit', (event) => {
        ...
        window.open(utils.buildWhatsAppUrl(`Hola Tulah, soy ${values.name}.${company}\nCorreo: ${values.email}\n\nSolicitud: ${values.message}`), '_blank', 'noopener,noreferrer');
      });
    }
  }
